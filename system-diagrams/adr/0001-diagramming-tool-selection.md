...
1. Support refactoring by visualizing dependencies
1. Are reviewable in PRs (text-based source)
...
1. **C4 Model**: Industry-standard architecture levels (Context → Container → Component)
1. **Automation**: Must support generation from `go list` output
1. **Low friction**: Easy to update when code changes
...
1. **C4 native support** — Built-in patterns for Context → Container → Component
1. **Live reload** — `d2 --watch` enables fast iteration
1. **Automation-friendly** — Simple syntax makes generation from `go list` straightforward
1. **Multiple output formats** — SVG for docs, PNG for README, PDF for presentations
...
1. Build `scripts/gen-deps.sh` for auto-generation
1. Add generated SVGs to `docs/` for GitHub rendering
1. Consider CI integration for diagram validation
